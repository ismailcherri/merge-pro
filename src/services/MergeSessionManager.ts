...
export class MergeSessionManager implements vscode.Disposable {
  ...
  async refreshFile(uri: vscode.Uri): Promise<void> {
    try {
      ...
      const mergedChunks = chunks.map((chunk, i) => {
        ...
        if (prev?.resolvedWith && this.chunkMatchesResolved(chunk, prev)) {
          return { ...chunk, resolvedWith: prev.resolvedWith, manualLines: prev.manualLines };
        }
        return chunk;
      });
      ...
      this.fileStates.set(uri.toString(), {
        ...
        resolvedChunks: mergedChunks.filter((c) => c.resolvedWith !== undefined).length,
        chunks: mergedChunks,
      });
    } catch {
  ...
  resolveChunk(uri: vscode.Uri, chunkIndex: number, decision: 'ours' | 'theirs' | 'manual', manualLines?: string[]): void {
