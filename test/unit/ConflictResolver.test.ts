...
describe('resolveFile', () => {
  ...
  it('replaces base lines with ours when resolved with ours', () => {
    const base = join('a', 'b', 'c');
    const chunk = makeChunk({ oursLines: ['OURS'], baseStartLine: 1, baseEndLine: 2, resolvedWith: 'ours' });
    expect(resolveFile(base, [chunk])).toBe(join('a', 'OURS', 'c'));
  });

  it('replaces base lines with theirs when resolved with theirs', () => {
    const base = join('a', 'b', 'c');
    const chunk = makeChunk({ theirsLines: ['THEIRS'], baseStartLine: 1, baseEndLine: 2, resolvedWith: 'theirs' });
    expect(resolveFile(base, [chunk])).toBe(join('a', 'THEIRS', 'c'));
  });

  it('uses manualLines when resolvedWith is manual', () => {
    const base = join('a', 'b', 'c');
    const chunk = makeChunk({ baseStartLine: 1, baseEndLine: 2, resolvedWith: 'manual', manualLines: ['MANUAL'] });
    expect(resolveFile(base, [chunk])).toBe(join('a', 'MANUAL', 'c'));
  });

  it('handles insertion (zero-length base range)', () => {
    const base = join('a', 'c');
    const chunk = makeChunk({ oursLines: ['b'], baseStartLine: 1, baseEndLine: 1, resolvedWith: 'ours' });
    expect(resolveFile(base, [chunk])).toBe(join('a', 'b', 'c'));
  });
  ...
  it('handles multiple chunks', () => {
    ...
    const chunks = [
      makeChunk({ oursLines: ['B'], baseStartLine: 1, baseEndLine: 2, resolvedWith: 'ours' }),
      makeChunk({ theirsLines: ['D'], baseStartLine: 3, baseEndLine: 4, resolvedWith: 'theirs' }),
    ];
